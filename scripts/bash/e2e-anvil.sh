...
#
# What it proves, per module: the production deploy script deploys the factory, the factory's
# create()/createDeterministic() produce correctly-wired modules and register them, PaymentRails
# routes real tokens through them, and the external protocol (Uniswap V3, CoW Protocol, Circle
# CCTP) accepts the resulting calls on real mainnet state.
#
...
deploy_script() { # <script-path> [--sig ... args] -> echoes the created contract address
  ...
  jq -r 'first(.transactions[] | select(.transactionType=="CREATE") | .contractAddress)' \
...
# ══════════════════════════════════════════════════════════════════════════════
phase "PHASE 2 — factory deployment (production deploy scripts)"

step "DeployPaymentRailsFactory"
RAILS_FACTORY=$(deploy_script scripts/solidity/deploy/DeployPaymentRailsFactory.s.sol)
step "DeployForwardModuleFactory"
FWD_FACTORY=$(deploy_script scripts/solidity/deploy/DeployForwardModuleFactory.s.sol)
step "DeployDexSwapModuleFactory (router=UniswapV3, no sequencer feed on L1)"
DEX_FACTORY=$(deploy_script scripts/solidity/deploy/DeployDexSwapModuleFactory.s.sol \
  --sig "run(address,address,uint256)" "$UNISWAP_V3_ROUTER" "0x0000000000000000000000000000000000000000" 0)
step "DeployCowSwapModuleFactory (GPv2Settlement)"
...
step "DeployCCTPBridgeModuleFactory (TokenMessengerV2 + USDC)"
CCTP_FACTORY=$(deploy_script scripts/solidity/deploy/DeployCCTPBridgeModuleFactory.s.sol \
  --sig "run(address,address)" "$TOKEN_MESSENGER_V2" "$USDC")

for pair in "PaymentRailsFactory:$RAILS_FACTORY" "ForwardModuleFactory:$FWD_FACTORY" \
            "DexSwapModuleFactory:$DEX_FACTORY" "CowSwapModuleFactory:$COW_FACTORY" \
            "CCTPBridgeModuleFactory:$CCTP_FACTORY"; do
  n=${pair%%:*}; a=${pair##*:}
...
step "factories carry the chain config they were deployed with"
assert_eq "DexSwapModuleFactory.router" "$(cu $DEX_FACTORY 'router()(address)')" "$UNISWAP_V3_ROUTER"
assert_eq "CowSwapModuleFactory.cowSettlement" "$(cu $COW_FACTORY 'cowSettlement()(address)')" "$GPV2_SETTLEMENT"
assert_eq "CCTPBridgeModuleFactory.tokenMessenger" "$(cu $CCTP_FACTORY 'tokenMessenger()(address)')" "$TOKEN_MESSENGER_V2"
assert_eq "CCTPBridgeModuleFactory.usdc" "$(cu $CCTP_FACTORY 'usdc()(address)')" "$USDC"
...
# ══════════════════════════════════════════════════════════════════════════════
phase "PHASE 4 — ForwardModule: factory + full lifecycle"

send "$DEPLOYER" "$FWD_FACTORY" "create()" || bad "ForwardModuleFactory.create()"
FWD_MODULE=$(addr_from_log "ForwardModule" "$FWD_FACTORY" "ForwardModuleCreated(address,address)")
ok "ForwardModule = $FWD_MODULE"
assert_eq "moduleType()" "$(cu $FWD_MODULE 'moduleType()(string)' | tr -d '"')" "FORWARD"

SALT_F=$(cast keccak "forward-e2e")
PREDICTED_F=$(cu $FWD_FACTORY "predictDeterministicAddress(bytes32)(address)" "$SALT_F")
send "$DEPLOYER" "$FWD_FACTORY" "createDeterministic(bytes32)" "$SALT_F" || bad "createDeterministic()"
FWD_MODULE_2=$(addr_from_log "ForwardModule#2" "$FWD_FACTORY" "ForwardModuleCreated(address,address)")
assert_eq "CREATE2 address matches prediction" "$FWD_MODULE_2" "$PREDICTED_F"
assert_eq "registry module count" "$(cu $FWD_FACTORY 'getModuleCount()(uint256)')" "2"
assert_eq "registry: isDeployedModule" "$(cu $FWD_FACTORY 'isDeployedModule(address)(bool)' $FWD_MODULE)" "true"
...
# ══════════════════════════════════════════════════════════════════════════════
phase "PHASE 5 — DexSwapModule: factory + full lifecycle (real Uniswap V3)"

send "$DEPLOYER" "$DEX_FACTORY" "create()" || bad "DexSwapModuleFactory.create()"
DEX_MODULE=$(addr_from_log "DexSwapModule" "$DEX_FACTORY" "DexSwapModuleCreated(address,address)")
ok "DexSwapModule = $DEX_MODULE"
assert_eq "module inherits factory router" "$(cu $DEX_MODULE 'router()(address)')" "$UNISWAP_V3_ROUTER"
...
SALT_D=$(cast keccak "dexswap-e2e")
PREDICTED_D=$(cu $DEX_FACTORY "predictDeterministicAddress(bytes32)(address)" "$SALT_D")
send "$DEPLOYER" "$DEX_FACTORY" "createDeterministic(bytes32)" "$SALT_D" || bad "createDeterministic()"
DEX_MODULE_2=$(addr_from_log "DexSwapModule#2" "$DEX_FACTORY" "DexSwapModuleCreated(address,address)")
assert_eq "CREATE2 address matches prediction" "$DEX_MODULE_2" "$PREDICTED_D"
assert_eq "registry module count" "$(cu $DEX_FACTORY 'getModuleCount()(uint256)')" "2"
...
# ══════════════════════════════════════════════════════════════════════════════
phase "PHASE 7 — CCTPBridgeModule: factory + full lifecycle (real Circle TokenMessengerV2)"

send "$DEPLOYER" "$CCTP_FACTORY" "create()" || bad "CCTPBridgeModuleFactory.create()"
CCTP_MODULE=$(addr_from_log "CCTPBridgeModule" "$CCTP_FACTORY" "CCTPBridgeModuleCreated(address,address)")
ok "CCTPBridgeModule = $CCTP_MODULE"
assert_eq "tokenMessenger wired from the factory" "$(cu $CCTP_MODULE 'tokenMessenger()(address)')" "$TOKEN_MESSENGER_V2"
assert_eq "usdc wired from the factory" "$(cu $CCTP_MODULE 'usdc()(address)')" "$USDC"
assert_eq "moduleType()" "$(cu $CCTP_MODULE 'moduleType()(string)' | tr -d '"')" "CCTP_BRIDGE"

SALT_B2=$(cast keccak "cctp-e2e")
PREDICTED_B2=$(cu $CCTP_FACTORY "predictDeterministicAddress(bytes32)(address)" "$SALT_B2")
send "$DEPLOYER" "$CCTP_FACTORY" "createDeterministic(bytes32)" "$SALT_B2" || bad "createDeterministic()"
CCTP_MODULE_2=$(addr_from_log "CCTPBridgeModule#2" "$CCTP_FACTORY" "CCTPBridgeModuleCreated(address,address)")
assert_eq "CREATE2 address matches prediction" "$CCTP_MODULE_2" "$PREDICTED_B2"
assert_eq "registry module count" "$(cu $CCTP_FACTORY 'getModuleCount()(uint256)')" "2"
...
printf "    ForwardModuleFactory     %s\n" "$FWD_FACTORY"
printf "    DexSwapModuleFactory     %s\n" "$DEX_FACTORY"
printf "    CowSwapModuleFactory     %s\n" "$COW_FACTORY"
printf "    CCTPBridgeModuleFactory  %s\n" "$CCTP_FACTORY"
